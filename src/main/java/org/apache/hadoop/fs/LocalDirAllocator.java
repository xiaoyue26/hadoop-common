...
import org.apache.hadoop.algorithm.*;
...
public class LocalDirAllocator {
  ...
  private static class AllocatorPerContext {
    ...
    private int dirNumLastAccessed; //random begin, every op set to random
    private Random dirIndexRandomizer = new Random();
    private FileSystem localFS; // FileSystem.getLocal(conf);
    private DF[] dirDF; // detail info of dir (command 'df -h')
    private DF[] ssdDF;

    private String contextCfgItemName; // primary key
    private String[] localDirs;// ../tmp1,../tmp2 // after filter
    private String[] ssdDirs; // after filter

    private String savedLocalDirs = ""; // conf.get(contextCfgItemName); //  original
    private String savedSsdDirs = "";

    private DirAlgo chwv;
    private String confInfo;
    public AllocatorPerContext(String contextCfgItemName) {
      this.contextCfgItemName = contextCfgItemName;
      // chwv =new RoundRobin(contextCfgItemName);
      chwv =new CHashJumpCapacity(contextCfgItemName);
      //DebugClient.print("new CHashJumpCapacity");
    }
    ...
    private synchronized boolean rebuildDir(int i,boolean[]ssdFlags,String conStr){
      ...
      DebugClient.print(conStr+" skip: "+ssdDirs[i]+":"+savedLocalDirs,"rebuild");
      return false;
    }
      ...
      try {
        if(localFS.mkdirs(tmpDir)|| localFS.exists(tmpDir)) {
          try {
            ...
            LOG.warn(inputDir + " is not writable\n", de);
          }
        } else {
          LOG.warn( "Failed to create " + inputDir);
        }
    ...
    private void updateHddDirs(int numHdd,boolean[]ssdFlags){
      try {
        ...
        /*String line=DebugClient.getArrayString(hddDirs);
        DebugClient.print(savedLocalDirs+"=>\n"+line);*/
    ...
    private synchronized void confChanged(Configuration conf)
      ...
      String newSsdDirs = conf.get("yarn.nodemanager.ssd-dirs");
      // debug:
      DebugClient.print("intermediate:"+conf.get("intermediate")+"====="+contextCfgItemName,"intermediate");
      confInfo="   "+contextCfgItemName+" :"+"==============";
      DebugClient.print(contextCfgItemName+"\n"+newLocalDirs,"context");
      //DebugClient.print("contextCfgItemName:"+contextCfgItemName);
      //DebugClient.print("newLocalDirs:"+newLocalDirs);
      //DebugClient.print("yarn.nodemanager.ssd-dirs:"+conf.get("yarn.nodemanager.ssd-dirs"));
      if (!newLocalDirs.equals(savedLocalDirs)) {
        localFS=FileSystem.getLocal(conf);
        configureHdd(newLocalDirs);
      }
      if (!newSsdDirs.equals(savedSsdDirs)) {
        localFS=FileSystem.getLocal(conf);
    ...
    private Path createPath(String path,
      ...
      Path file= new Path(new Path(localDirs[dirNumLastAccessed]),
                path);
      DebugClient.print("old_createPath:"+file);
      if (checkWrite) {
    ...
    private Path createPath(String dir,String path,
                            boolean checkWrite) throws IOException {
     ...
     */
    int getCurrentDirectoryIndex() {
      DebugClient.print("getCurrentDirectoryIndex:"+ localDirs[dirNumLastAccessed]);
      return dirNumLastAccessed;
    }

    private Path rouletteSelection(int numDirsSearched
            ,int numDirs,String pathStr, boolean checkWrite)
            throws IOException {
      ...
      while (numDirsSearched < numDirs && returnPath == null) {
        ...
        int dirNo = 0;
        while (randomPosition > availableOnDisk[dirNo]) {
          randomPosition -= availableOnDisk[dirNo];
          dirNo++;
        }
        dirNumLastAccessed = dirNo;
        returnPath = createPath(pathStr, checkWrite);
        //DebugClient.print("getLocalPathForWrite:SIZE_UNKNOWN:"+returnPath);
        if (returnPath == null) {
          totalAvailable -= availableOnDisk[dirNo];
          availableOnDisk[dirNo] = 0; // skip this disk
          numDirsSearched++;
        }
      }
      return returnPath;
    ...
    public synchronized Path getLocalPathForWrite(String pathStr, long size,
      ...
      confChanged(conf);
      //DebugClient.print("try getLocalPathForWrite");
      /*int numDirs = localDirs.length;
      ...
      if(size == SIZE_UNKNOWN) {  //do roulette selection: pick dir with probability
        ...
        // returnPath = rouletteSelection(numDirsSearched,numDirs,pathStr,checkWrite);
        String dir=chwv.getDir(pathStr,size);
        // dirNumLastAccessed?
        returnPath = createPath(dir,pathStr,checkWrite);
        DebugClient.print(confInfo+"getLocalPathForWrite:SIZE_UNKNOWN:"+returnPath);
      } else {
        // here
        String dir=chwv.getDir(pathStr,size);
        // dirNumLastAccessed?
        returnPath = createPath(dir,pathStr,checkWrite);
        DebugClient.print(confInfo+"getLocalPathForWrite:SIZE_KNOWN:"+returnPath);
      }
      if (returnPath != null) {
        return returnPath;
      }
     ...
     * {LocalDirAllocator.SIZE_UNKNOWN} if not known apriori. We
     *  round-robin over the set of disks (via the configured dirs) and return
    ...
    public synchronized Path getLocalPathToRead(String pathStr,
        Configuration conf) throws IOException {
      confChanged(conf);
      DebugClient.print(confInfo+"ReadRequest:"+ pathStr);
      //remove the leading slash from the path (to make sure that the uri
      //resolution results in a valid path on the dir being checked)
      if (pathStr.startsWith("/")) {
        pathStr = pathStr.substring(1);
      }
      Path file= chwv.find(pathStr,localFS);
      if(file!=null) {
          DebugClient.print(confInfo+"ReadReturn:"+ file);
          return file;
      }
      DebugClient.print(confInfo+"couldn't find:"+ pathStr);
      //no path found
    ...
    synchronized Iterable<Path> getAllLocalPathsToRead(String pathStr,
        Configuration conf) throws IOException {
      confChanged(conf);
      DebugClient.print("try getAllLocalPathsToRead");
      if (pathStr.startsWith("/")) {
        pathStr = pathStr.substring(1);
      }
    ...
    public synchronized boolean ifExists(String pathStr,Configuration conf) {
        DebugClient.print("ifExists:"+ pathStr);
        //remove the leading slash from the path (to make sure that the uri
        //resolution results in a valid path on the dir being checked)
        if (pathStr.startsWith("/")) {
          pathStr = pathStr.substring(1);
        ...
        try {
          return chwv.ifExists(pathStr,localFS);
        }
        catch (Exception e){
           DebugClient.print("ifExists error");
        }
        return false;
    }
